...
if [ -f "$E2E_DIR/create_fake_data.sh" ]; then
    chmod +x "$E2E_DIR/create_fake_data.sh"
    "$E2E_DIR/create_fake_data.sh"
    echo "✅ Fake data created successfully"
else
    echo "⚠️  create_fake_data.sh not found in $E2E_DIR"
fi
