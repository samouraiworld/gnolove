...
const ProposalDetail = ({ id }: { id: string }) => {
  ...
  const forPct = percent(totals.for, totalEligible);
  const againstPct = percent(totals.against, totalEligible);
  const abstainPct = percent(totals.abstain, totalEligible);
  ...
  return (
    <Flex direction="column" gap="4" pt="6">
      <Flex align={{ initial: 'start', md: 'center' }} direction={{ initial: 'column', md: 'row' }} justify="between">
        ...
        <Text color="gray" size="2" mt={{ initial: '2', md: '0' }}>
          ID: {proposal.id}
        </Text>
      </Flex>
              ...
              <Tabs.Content value="breakdown">
                <Flex direction="column" gap="3">
                  <Text mb="1" color="gray">
                    Voting Progress ({totals.total}/{totalEligible} voted)
                  </Text>
                  <Box className="relative h-2 w-full overflow-hidden rounded-full bg-gray-6">
                    <Box
                      className="absolute left-0 top-0 h-full bg-green-9"
                      style={{ width: `${Math.min(100, forPct)}%` }}
                    />
                  </Box>
                  <Flex justify="between">
                    <Text color="green" size="2">
                      For {forPct}% ({totals.for})
                    </Text>
                    <Text color="gray" size="2">
                      Abstain {abstainPct}% ({totals.abstain})
                    </Text>
                    <Text color="red" size="2">
                      Against {againstPct}% ({totals.against})
                    </Text>
                  </Flex>
                </Flex>
              </Tabs.Content>
                      ...
                      {[...votes]
                        ...
                        .map((v) => {
                          const color: BadgeProps['color'] =
                            v.vote === 'YES' ? 'green' : v.vote === 'NO' ? 'red' : 'gray';
                          return (
                  ...
                  {(proposal.files || []).map((f) => (
                    <Box key={f.id}>
                      ...
                      {(() => {
                        ...
                        if (lang === 'markdown') {
                          return (
                            <Box className="rounded-md text-white overflow-hidden bg-[#0b0b0c] p-4">
        ...
        <Card className="col-span-full md:col-span-1">
          <Flex direction="column" gap="3">
            ...
            <DetailRow
              ...
              value={proposal.executionHeight ? String(proposal.executionHeight) : '-'}
            />
